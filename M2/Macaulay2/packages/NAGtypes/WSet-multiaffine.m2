...
dim WCollection := W -> ( 
    dims := witnessedDimensions W; 
    if #dims>0 then max\\sum\dims
    )
witnessedDimensions = method()
witnessedDimensions WCollection := W ->  keys W#"witnesses" 
codim WCollection := {} >> o -> W ->  apply(dim W, d -> dim ambient W - d)
ambient WCollection := W -> W#"ambient"
equations WCollection := W -> W#"equations" 
...
addWSet (WCollection, MultiSlicingVariety, List) := (W,S,pts) ->(
    ...
    if not W#"witnesses"#?(codim S) then W#"witnesses"#(codim S) = {};
